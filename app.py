...
# --- 網頁配置 ---
st.set_page_config(page_title="AI股票健診雷達", page_icon="🚀", layout="wide")
...
if st.sidebar.button("啟動健診分析 🎯", type="primary") or ticker_input:
    with st.spinner(f'正在進行量化分析 {ticker_input} ...'):
        ...
        if df is not None and not df.empty:
            ...
            # 防呆機制：若上市時間太短，算完均線後資料變空
            if df.empty:
                st.warning(f"⚠️ {ticker_input} 歷史數據不足以計算技術指標 (需至少 60 個交易日，可能為近期新上市股票或遇到 API 抓取限制)，請嘗試換一檔股票。")
            else:
                # 取得最新數據
                ...
                pct_change = (price_change / float(df['Close'].iloc[-2])) * 100 if len(df) > 1 else 0
                ...
                # 取代了過期的 use_container_width=True 設定
