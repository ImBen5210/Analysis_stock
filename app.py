...
if st.sidebar.button("啟動健診分析 🎯", type="primary") or ticker_input:
    with st.spinner(f'正在進行量化分析 {ticker_input} ...'):
        ...
        if df is not None and not df.empty:
            ...
            pct_change = (price_change / float(df['Close'].iloc[-2])) * 100 if float(df['Close'].iloc[-2]) != 0 else 0
            ...
            if peg_ratio is not None:
                col3.metric("PEG (本益成長比)", f"{peg_ratio:.2f}", 
                           ...
                           delta_color="inverse" if peg_ratio >= 1 else "normal")
            ...
            if peg_ratio is not None:
                ...
                details.append(f"🔵 **估值評估 (PEG)**：無法取得成長數據（{ticker_input} 常見），Trailing P/E ≈ {pe_text}，給予中立分數 8.0/20。")
